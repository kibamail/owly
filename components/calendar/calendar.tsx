...
import { useDatePicker, type DPDay, type DPUserConfig } from '@rehookify/datepicker'
 ...
 return (
  <div className="kb-calendar-root">
   ...
   <div className="kb-calendar-header">
    <Button variant='secondary' size='xs' className='kb-calendar-header-button' {...subtractOffset({ months: 1 })}>
     <NavArrowLeftIcon />
    </Button>

    <Text className='kb-calendar-header-label'>{month} {year}</Text>

    <Button variant='secondary' size='xs' className='kb-calendar-header-button' {...addOffset({ months: 1 })}>
     <NavArrowRightIcon />
    </Button>
   </div>
   ...
   <div className="kb-calendar-week-days">
    {weekDays.map((day) => (
     <span className="kb-calendar-week-day" key={day}>{day.slice(0, -1)}</span>
    ))}
   </div>

   <div className="kb-calendar-days-grid">
    {days.map((day) => (
     <button className={getDayClassName(day)} key={day.$date.toString()} {...dayButton(day)}>
      <Text>{day.day}</Text>
     </button>
    ))}
   </div>
